...
class LoginFormTestCase:
    ...
    def test_blank_username_authentication(self):
    ...
    def test_blank_password_authentication(self):
        form_input = {'username': 'jd123', 'password': ''}
        form = LogInForm(data=form_input)
        user = form.get_user()
        self.assertEqual(user, None)

    def test_form_incorrect_username(self):
        self.form_input['username'] = 'ja'
        form = LogInForm(data=self.form_input)
        self.assertTrue(form.is_valid())

    def test_form_incorrect_password(self):
        self.form_input['password'] = 'pwd'
        form = LogInForm(data=self.form_input)
        self.assertTrue(form.is_valid())

    def test_authenticate_valid_user(self):
        fixture = User.objects.get(username='jd123')
        form_input = {'username': 'jd123', 'password': 'Password123'}
        form = LogInForm(data=form_input)
        user = form.get_user()
        self.assertEqual(user, fixture)

    def test_invalid_credentials(self):
        form_input = {'username': 'jd123', 'password': 'WrongPassword123'}
        form = LogInForm(data=form_input)
        user = form.get_user()
        self.assertEqual(user, None)
